...
<div class="appointment-detail">
    ...
    <div class="appointment-detail-grid">
        ...
        <aside class="appointment-context-panels" aria-label="<?= __('Appointment details') ?>">
            <section class="appointment-panel" aria-labelledby="appointment-organisation-heading">
                <p class="appointment-eyebrow"><?= __('Organisation') ?></p>
                <h2 id="appointment-organisation-heading"><?= __('Group, team & section') ?></h2>
                <dl class="appointment-metadata">
                    <dt><?= __('Group') ?></dt>
                    <dd><?php if ($team?->group) : ?>
                        <?= $this->Html->link($team->group->group_name, ['controller' => 'Groups', 'action' => 'view', $team->group->id]) ?>
                        <?php else : ?>
                        <?= __('Not assigned') ?>
                    <?php endif; ?></dd>
                    <dt><?= __('Team') ?></dt>
                    ...
                    <?php endif; ?></dd>
                    <dt><?= __('Section') ?></dt>
                    <dd><?= $team?->section ? h($team->section->section_name) : __('Not assigned') ?></dd>
                </dl>
            </section>
            ...
            <section class="appointment-panel<?= $appointment->member_contact_method?->is_non_group_email ? ' appointment-non-group-email' : '' ?>"><h2><?= __('Contact method') ?></h2><?php if ($appointment->member_contact_method) : ?><p class="appointment-contact-type"><?= h($appointment->member_contact_method->contact_method_type->label()) ?></p><p class="appointment-contact-value"><?= h($appointment->member_contact_method->contact_method) ?></p><?php if ($appointment->member_contact_method->is_non_group_email) : ?><p class="appointment-email-warning"><?= __('Non-group email') ?></p><?php endif; ?><?php else : ?><p class="appointment-empty"><?= __('No contact method is assigned.') ?></p><?php endif; ?></section>
